        ...
        when {
            ...
            userName == LCGApp.context.getString(R.string.login_or_register) -> {
                ...
                return UserInfo(
                    ...
                    avatarUrl = avatar,
                    groupInfo = groupInfo,
                    wuaiCoin = infoList.getOrNull(3)?.text().orEmpty(),
                    credit = infoList.getOrNull(1)?.text().orEmpty(),
                    answerRate = infoList.getOrNull(6)?.text().orEmpty(),
                    enthusiasticValue = infoList.getOrNull(7)?.text().orEmpty(),
...
fun parseNotificationInfo(doc: Document): NotificationInfo {
    with(doc) {
        ...
        menu?.select("li")?.forEach {
            try {
                it.select("a > span").firstOrNull()?.text()?.also { v ->
